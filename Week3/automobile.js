...
for that comparator and it will return a new array 
...
function sortArr( comparator, array )
{
    ...
    while (loop_indicator) 
	{
		...
		{
            if (comparator(result[i], result[i-1])) //Bubble
			{
                var temp = result[i]; 
                result[i] = result[i-1];
                result[i-1] = temp; 
...
function typeComparator( auto1, auto2)
{
	...
	if (types.indexOf(auto1.type.toLowerCase()) > types.indexOf(auto2.type.toLowerCase())) 
	{
		return true;
    } 
	else if (types.indexOf(auto1.type.toLowerCase()) == types.indexOf(auto2.type.toLowerCase())) 
	{
...
it prints "year make model type" with the year, make, model and type being the values appropriate for the automobile. 
...
function showResult()
{
	...
	var b;	// print type or not
	var sort_type = [sortArr(yearComparator, automobiles), 
	...
	for (var i = 0; i < 3; i++) 
	{
		...
		for (var j = 0; j < automobiles.length; j++)
		{
			sort_type[i][j].logMe(b);
		}
